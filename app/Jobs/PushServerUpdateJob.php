...
class PushServerUpdateJob implements ShouldBeEncrypted, ShouldQueue, Silenced
{
    ...
    public function handle()
    {
        ...
        foreach ($this->containers as $container) {
            ...
            $coolify_managed = $labels->has('deployflow.managed');
            ...
            if ($name === 'deployflow-log-drain' && $this->isRunning($containerStatus)) {
                $this->foundLogDrainContainer = true;
            }
            if ($labels->has('deployflow.applicationId')) {
                $applicationId = $labels->get('deployflow.applicationId');
                $pullRequestId = $labels->get('deployflow.pullRequestId', '0');
                try {
                    if ($pullRequestId === '0') {
            ...
            } elseif ($labels->has('deployflow.serviceId')) {
                $serviceId = $labels->get('deployflow.serviceId');
                $subType = $labels->get('deployflow.service.subType');
                $subId = $labels->get('deployflow.service.subId');
                ...
                $type = $labels->get('deployflow.type');
                if ($name === 'deployflow-proxy' && $this->isRunning($containerStatus)) {
            ...
            if ($this->foundProxy === false) {
                try {
                    if (CheckProxy::run($this->server)) {
                        ...
                        $this->server->team?->notify(new ContainerRestarted('deployflow-proxy', $this->server));
                    }
                } catch (\Throwable $e) {
                }
            } else {
