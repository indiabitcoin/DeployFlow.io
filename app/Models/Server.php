   ...
   redir $redirect_url
    ...
    public function setupDynamicProxyConfiguration()
    {
        ...
        if ($this->proxyType() === ProxyTypes::TRAEFIK->value) {
            $file = "$dynamic_config_path/deployflow.yaml";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                ...
                $traefik_dynamic_conf = [
                    'http' => [
                        ...
                        'routers' => [
                            ...
                            'deployflow-realtime-ws' => [
                                'entryPoints' => [
                                    0 => 'http',
                                ],
                                'service' => 'deployflow-realtime',
                        ...
                        'services' => [
                            ...
                            'deployflow-realtime' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://deployflow-realtime:6001',
                                        ],
                                    ],
                                ],
                            ],
                            'coolify-terminal' => [
                                'loadBalancer' => [
                                    'servers' => [
                                        0 => [
                                            'url' => 'http://deployflow-realtime:6002',
                                        ],
                                    ],
                                ],
                            ],
                ...
                if ($schema === 'https') {
                    ...
                    $traefik_dynamic_conf['http']['routers']['deployflow-realtime-wss'] = [
                        'entryPoints' => [
                            0 => 'https',
                        ],
                        'service' => 'deployflow-realtime',
            ...
            $file = "$dynamic_config_path/deployflow.caddy";
            if (empty($settings->fqdn) || (isCloud() && $this->id !== 0) || ! $this->isLocalhost()) {
                instant_remote_process([
                    "rm -f $file",
                ], $this);
...
$schema://$host {
    handle /app/* {
        reverse_proxy deployflow-realtime:6001
    }
    handle /terminal/ws {
        reverse_proxy deployflow-realtime:6002
    }
    ...
    public function reloadCaddy()
    {
        return instant_remote_process([
            'docker exec deployflow-proxy caddy reload --config /config/caddy/Caddyfile.autosave',
        ], $this);
    }

    public function proxyPath()
    {
        $base_path = config('constants.deployflow.base_config_path');
    ...
    public function getCpuMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            ...
            $cpu = instant_remote_process(["docker exec deployflow-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->sentinel_token}\" http://localhost:8888/api/cpu/history?from=$from'"], $this, false);
    ...
    public function getMemoryMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            $from = now()->subMinutes($mins)->toIso8601ZuluString();
            $memory = instant_remote_process(["docker exec deployflow-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->sentinel_token}\" http://localhost:8888/api/memory/history?from=$from'"], $this, false);
    ...
    public function loadUnmanagedContainers(): Collection
    {
        if ($this->isFunctional()) {
            ...
            $containers = $containers->map(function ($container) {
                ...
                if (! str($labels)->contains('deployflow.managed')) {
                    return $container;
                }
    ...
    public function generateCaCertificate()
    {
        try {
            ...
            if ($caCertificate) {
                ...
                $caCertPath = config('constants.deployflow.base_config_path').'/ssl/';
