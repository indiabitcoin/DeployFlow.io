...
class Init extends Command
{
    ...
    public function handle()
    {
        ...
        if (! is_null(config('constants.deployflow.autoupdate', null))) {
            if (config('constants.deployflow.autoupdate') == true) {
        ...
        foreach ($this->servers as $server) {
            ...
            try {
                ...
                foreach ($removeNetworks as $network) {
                    ...
                    if (empty($out)) {
                        $commands->push("docker network disconnect $network deployflow-proxy >/dev/null 2>&1 || true");
                        $commands->push("docker network rm $network >/dev/null 2>&1 || true");
                    } else {
                        $data = collect(json_decode($out, true));
                        if ($data->count() === 1) {
                            // If only deployflow-proxy itself is connected to that network (it should not be possible, but who knows)
                            $isCoolifyProxyItself = data_get($data->first(), 'Name') === 'deployflow-proxy';
    ...
    {
        if (version_compare('4.0.0-beta.179', config('constants.deployflow.version'), '<=')) {
            try {
    ...
    {
        $id = config('app.id');
        $version = config('constants.deployflow.version');
        try {
    ...
    {
        if (version_compare('4.0.0-beta.298', config('constants.deployflow.version'), '<=')) {
