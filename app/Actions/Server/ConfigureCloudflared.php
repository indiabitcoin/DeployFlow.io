...
class ConfigureCloudflared
{
    ...
    public function handle(Server $server, string $cloudflare_token, string $ssh_domain): Activity
    {
        try {
            $config = [
                'services' => [
                    'deployflow-cloudflared' => [
                        'container_name' => 'deployflow-cloudflared',
            ...
            $commands = collect([
                ...
                'docker rm -f deployflow-cloudflared || true',
                'echo Starting new Cloudflare Tunnel container.',
                'docker compose up --wait --wait-timeout 15 --remove-orphans || docker logs deployflow-cloudflared',
            ]);
