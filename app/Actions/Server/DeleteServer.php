...
class DeleteServer
{
    ...
    public function handle(int $serverId, bool $deleteFromHetzner = false, ?int $hetznerServerId = null, ?int $cloudProviderTokenId = null, ?int $teamId = null)
    {
        ...
        // Delete from Hetzner even if server is already gone from DeployFlow
        ...
        ray($server ? 'Deleting server from DeployFlow' : 'Server already deleted from DeployFlow, skipping DeployFlow deletion');

        // If server is already deleted from DeployFlow, skip this part
        if (! $server) {
