...
class ComplexStatusCheck
{
    ...
    public function handle(Application $application)
    {
        ...
        foreach ($servers as $server) {
            ...
            $containers = instant_remote_process(["docker container inspect $(docker container ls -q --filter 'label=deployflow.applicationId={$application->id}' --filter 'label=deployflow.pullRequestId=0') --format '{{json .}}'"], $server, false);
