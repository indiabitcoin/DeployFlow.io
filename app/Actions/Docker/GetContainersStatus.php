    ...
    public function handle(Server $server, ?Collection $containers = null, ?Collection $containerReplicates = null)
    {
        ...
        foreach ($this->containers as $container) {
            if ($this->server->isSwarm()) {
                ...
                $uuid = data_get($labels, 'deployflow.name');
            } else {
            ...
            $applicationId = data_get($labels, 'deployflow.applicationId');
            if ($applicationId) {
                $pullRequestId = data_get($labels, 'deployflow.pullRequestId');
                if ($pullRequestId) {
                ...
                $type = data_get($labels, 'deployflow.type');

                if ($uuid) {
                    if ($type === 'service') {
                        $database_id = data_get($labels, 'deployflow.service.subId');
                        if ($database_id) {
                                ...
                                if ($uuid) {
                                    ...
                                    if ($isPublic) {
                                        $foundTcpProxy = $this->containers->filter(function ($value, $key) use ($uuid) {
                                            if ($this->server->isSwarm()) {
                                                return data_get($value, 'Spec.Name') === "deployflow-proxy_$uuid";
                                            } else {
            ...
            $serviceLabelId = data_get($labels, 'deployflow.serviceId');
            if ($serviceLabelId) {
                $subType = data_get($labels, 'deployflow.service.subType');
                $subId = data_get($labels, 'deployflow.service.subId');
