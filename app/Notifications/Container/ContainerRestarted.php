...
class ContainerRestarted extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("DeployFlow: A resource ({$this->name}) has been restarted automatically on {$this->server->name}");
    ...
    {
        $message = "DeployFlow: A resource ({$this->name}) has been restarted automatically on {$this->server->name}";
        $payload = [
            'message' => $message,
        ];
        if ($this->url) {
            $payload['buttons'] = [
                [
                    [
                        'text' => 'Check Proxy in DeployFlow',
                        'url' => $this->url,
                    ],
                ],
            ];
