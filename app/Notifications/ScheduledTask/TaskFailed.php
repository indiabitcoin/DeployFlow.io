...
class TaskFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("DeployFlow: [ACTION REQUIRED] Scheduled task ({$this->task->name}) failed.");
    ...
    {
        $message = "DeployFlow: Scheduled task ({$this->task->name}) failed with output: {$this->output}";
        if ($this->url) {
            $buttons[] = [
                'text' => 'Open task in DeployFlow',
                'url' => (string) $this->url,
            ];
        }
