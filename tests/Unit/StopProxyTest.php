...
it('ensures stop proxy includes wait loop for container removal', function () {
    ...
    $commands = [
        'docker stop --time=30 deployflow-proxy 2>/dev/null || true',
    ...
    expect($commandsString)->toContain('docker stop --time=30 deployflow-proxy')
        ->and($commandsString)->toContain('docker rm -f deployflow-proxy')
        ->and($commandsString)->toContain('for i in {1..10}; do')
        ->and($commandsString)->toContain('if ! docker ps -a --format "{{.Names}}" | grep -q "^deployflow-proxy$"')
...
it('uses configurable timeout for docker stop', function () {
    ...
    $timeout = 30;
    $stopCommand = "docker stop --time=$timeout deployflow-proxy 2>/dev/null || true";
...
it('waits for swarm service container removal correctly', function () {
    ...
    $containerName = 'deployflow-proxy_traefik';
    $checkCommand = "    if ! docker ps -a --format \"{{.Names}}\" | grep -q \"^$containerName$\"; then";

    expect($checkCommand)->toContain('deployflow-proxy_traefik');
});
