...
function collectProxyDockerNetworksByServer(Server $server)
{
    ...
    $networks = instant_remote_process(['docker inspect --format="{{json .NetworkSettings.Networks }}" deployflow-proxy'], $server, false);
    ...
    if ($server->isSwarm()) {
        $commands = $networks->map(function ($network) {
            return [
                ...
                "docker network connect $network deployflow-proxy >/dev/null 2>&1 || true",
                "echo 'Successfully connected deployflow-proxy to $network network.'",
            ];
        });
    } else {
        $commands = $networks->map(function ($network) {
...
function generateDefaultProxyConfiguration(Server $server, array $custom_commands = [])
{
    ...
    if ($proxy_type === ProxyTypes::TRAEFIK->value) {
        $labels = [
            ...
            'deployflow.proxy=true',
        ];
        $config = [
            'name' => 'deployflow-proxy',
            'networks' => $array_of_networks->toArray(),
            'services' => [
                'traefik' => [
                    'container_name' => 'deployflow-proxy',
                    'image' => 'traefik:v3.6',
...
function getExactTraefikVersionFromContainer(Server $server): ?string
{
    try {
        ...
        $versionCommand = "docker exec deployflow-proxy traefik version 2>/dev/null | grep -oP 'Version:\s+\K\d+\.\d+\.\d+'";
        ...
        $labelCommand = "docker inspect deployflow-proxy --format '{{index .Config.Labels \"org.opencontainers.image.version\"}}' 2>/dev/null";
...
function getTraefikVersionFromDockerCompose(Server $server): ?string
{
    try {
        ...
        $containerName = 'deployflow-proxy';
