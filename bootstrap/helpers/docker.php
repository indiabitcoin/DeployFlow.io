...
function getCurrentApplicationContainerStatus(Server $server, int $id, ?int $pullRequestId = null, ?bool $includePullrequests = false): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=deployflow.applicationId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);

        $containers = $containers->map(function ($container) use ($pullRequestId, $includePullrequests) {
            $labels = data_get($container, 'Labels');
            $containerName = data_get($container, 'Names');
            $hasPrLabel = str($labels)->contains('deployflow.pullRequestId=');
            $prLabelValue = null;
            ...
            if ($pullRequestId !== null && $pullRequestId !== 0 && str($labels)->contains("deployflow.pullRequestId={$pullRequestId}")) {
                return $container;
            }
...
function getCurrentServiceContainerStatus(Server $server, int $id): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=deployflow.serviceId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);
...
{
    $labels = collect([]);
    $labels->push('deployflow.managed=true');
    $labels->push('deployflow.type=database');
    $labels->push('deployflow.databaseId='.$database->id);
    $labels->push('deployflow.resourceName='.Str::slug($database->name));
    $labels->push('deployflow.serviceName='.Str::slug($database->name));
    $labels->push('deployflow.projectName='.Str::slug($database->project()->name));
    $labels->push('deployflow.environmentName='.Str::slug($database->environment->name));
    $labels->push('deployflow.database.subType='.$database->type());
...
function defaultLabels($id, $name, string $projectName, string $resourceName, string $environment, $pull_request_id = 0, string $type = 'application', $subType = null, $subId = null, $subName = null)
{
    $labels = collect([]);
    $labels->push('deployflow.managed=true');
    $labels->push('deployflow.version='.config('constants.deployflow.version'));
    $labels->push('deployflow.'.$type.'Id='.$id);
    $labels->push("deployflow.type=$type");
    $labels->push('deployflow.name='.$name);
    $labels->push('deployflow.resourceName='.Str::slug($resourceName));
    $labels->push('deployflow.projectName='.Str::slug($projectName));
    $labels->push('deployflow.serviceName='.Str::slug($subName ?? $resourceName));
    $labels->push('deployflow.environmentName='.Str::slug($environment));

    $labels->push('deployflow.pullRequestId='.$pull_request_id);
    if ($type === 'service') {
        $subId && $labels->push('deployflow.service.subId='.$subId);
        $subType && $labels->push('deployflow.service.subType='.$subType);
        $subName && $labels->push('deployflow.service.subName='.Str::slug($subName));
    }
