...
{
    return ! config('constants.deployflow.self_hosted');
}
...
function generateFqdn(Server $server, string $random, bool $forceHttps = false, int $parserVersion = 5): string
{
    ...
    if ($parserVersion >= 5 && version_compare(config('constants.deployflow.version'), '4.0.0-beta.420.7', '>=')) {
...
function getHelperVersion(): string
{
    ...
    return config('constants.deployflow.helper_version');
}
...
function loadConfigFromGit(string $repository, string $branch, string $base_directory, int $server_id, int $team_id)
{
    ...
    $fileList = collect([".$workdir/deployflow.json"]);
    $commands = collect([
        "rm -rf /tmp/{$uuid}",
        "mkdir -p /tmp/{$uuid}",
        "cd /tmp/{$uuid}",
